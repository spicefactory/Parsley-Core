...
/**
 * API for configuring methods which wish to be invoked when
 * a matching asynchronous command execution aborts with an error.
 * 
 * @author Jens Halm
 */
public class CommandError {
	 ...
	 * @private
	 */
	function CommandError (method: String) {
		this.method = method;
	}
	...
	public function apply (builder: ObjectDefinitionBuilder) : void {
		...
		var factory:Function = function (): Object {
			return new DefaultCommandObserver(info, MessageReceiverKind.COMMAND_ERROR_BY_TRIGGER);
		};
	...
	public static function forMethod (method:String) : CommandError {
		return new CommandError(method);
	}
